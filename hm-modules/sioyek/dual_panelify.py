...
if __name__ == "__main__":
    ...
    for i in range(start_index, doc.page_count - 1, 2):
        if (datetime.datetime.now() - last_update_time).seconds > UPDATE_EVERY_SECONDS:
            ...
            sioyek.set_status_string(
                "Dual panelifying {} / {}".format(i + 1, doc.page_count)
            )
        ...
        total_width = (
            page1.rect.width + page2.rect.width + 2 * SIDE_MARGIN + MIDDLE_MARGIN
        )
        total_height = max(page1.rect.height, page2.rect.height)
        new_page = pdf_writer.new_page(width=total_width, height=total_height)

        left_rect = fitz.Rect(
            SIDE_MARGIN, 0, SIDE_MARGIN + page1.rect.width, page1.rect.height
        )
    ...
    if (doc.page_count - start_index) % 2 == 1:
        pdf_writer.insert_pdf(
            doc, from_page=doc.page_count - 1, to_page=doc.page_count - 1
        )
